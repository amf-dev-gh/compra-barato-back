package site.amf_dev.carrefour.services;

import com.microsoft.playwright.*;
import com.microsoft.playwright.options.LoadState;

import site.amf_dev.carrefour.dtos.ProductDto;
import site.amf_dev.carrefour.exceptions.ScrapingException;
...
@Service
public class CarrefourScraperService {

	private static final Logger logger = LoggerFactory.getLogger(CarrefourScraperService.class);
	...
	public List<ProductDto> searchProducts(String query, int maxPages) {
		...
		try (Playwright playwright = Playwright.create()) {
			...
			try {
				...
				// Navegar a Carrefour y buscar productos
			...
			throw new ScrapingException("Error al realizar scraping en Carrefour", e);
		}
	...
	private Browser createBrowser(Playwright playwright) {
		BrowserType.LaunchOptions launchOptions = new BrowserType.LaunchOptions().setChannel("chrome").setHeadless(headless).setTimeout(timeout)
	...
	private void navigateToPage(Page page, String query) {
		...
		while (retries < maxRetries) {
			try {
				logger.info("Navegando a Carrefour (intento {})", retries + 1);
				page.navigate("https://www.carrefour.es/?gad_source=1&filter=sold_by_c4%3Atrue&query=" + query);
				page.waitForLoadState(LoadState.NETWORKIDLE);
				humanLikeDelay();
				return;
			} catch (Exception e) {
				retries++;
				logger.warn("Error al navegar a Carrefour, intento {}: {}", retries, e.getMessage());
				if (retries >= maxRetries) {
					throw new ScrapingException("No se pudo navegar a Carrefour después de " + maxRetries + " intentos",
							e);
				}
				humanLikeDelay();
			}
	...
	private void acceptCookies(Page page) {
		try {
			...
			String cookieSelector = "#onetrust-accept-btn-handler";

			try {
				if (page.locator(cookieSelector).isVisible()) {
					page.locator(cookieSelector).click();
					logger.info("Cookies aceptadas con selector: {}", cookieSelector);
					humanLikeDelay();
					return;
				}
			} catch (Exception e) {
				logger.debug("Selector de cookies no encontrado: {}", cookieSelector);
			}
	...
	private List<ProductDto> extractProductsFromCurrentPage(Page page) {
		...
		try {
			...
			page.waitForSelector("article, li", new Page.WaitForSelectorOptions().setTimeout(waitForSelectorTimeout));

			// Diferentes selectores posibles para productos
			String[] productSelectors = { "li.x-base-grid__result", "[data-test='search-grid-result']",
					".x-base-grid__item" };
	...
	private ProductDto extractProductInfo(Locator productElement) {
		try {
			...
			// Extraer nombre
			try {
				String name = extractText(productElement, "[data-test='result-title']", "a.x-result-link");
			...
			// Extraer precio
			try {
				String price = extractText(productElement, "[data-test='result-current-price']", "span.x-currency");
			...
			// Extraer imagen
			try {
				String imageUrl = extractImageUrl(productElement, "[data-test='result-picture-image']",
						"img.x-result-picture-image");
			...
			try {
				String productDescription = extractText(productElement, ".x-text2", ".x-text2-sm",
						".x-text-neutral-60");
				product.setDescription(productDescription);
			} catch (Exception e) {
				logger.debug("No se pudo extraer la Descripción del producto");
			}

			// Solo devolver el producto si tiene al menos nombre y precio
			if (product.getName() != null && product.getPrice() != null) {
				product.setStore("Carrefour");
				return product;
			}
			...
			try {
				...
				if (element.isVisible()) {
					...
					if (src != null && !src.isEmpty()) {
						return src.startsWith("http") ? src : "https://www.carrefour.es/" + src;
					}
				}
			} catch (Exception e) {
	...
	private boolean hasNextPage(Page page) {
		try {
			String[] nextPageSelectors = { ".pagination-next:not(.disabled)", "[aria-label*='Load']:not(.disabled)",
					"button:has-text('Cargar más resultados'):not(.disabled)" };
