package site.amf_dev.consum.services;
...
@Service
public class ConsumScraperService {

	private static final Logger logger = LoggerFactory.getLogger(ConsumScraperService.class);
	...
	public List<ProductDto> searchProducts(String query, int maxPages) {
		...
		try (Playwright playwright = Playwright.create()) {
			...
			try {
				...
				// Navegar a Consum y buscar productos
			...
			throw new ScrapingException("Error al realizar scraping en Consum", e);
		}
	...
	private void navigateToPage(Page page, String query) {
		...
		while (retries < maxRetries) {
			try {
				logger.info("Navegando a Consum (intento {})", retries + 1);
				page.navigate("https://tienda.consum.es/es/s/" + query);
				page.waitForLoadState(LoadState.NETWORKIDLE);
				humanLikeDelay();
				return;
			} catch (Exception e) {
				retries++;
				logger.warn("Error al navegar a Consum, intento {}: {}", retries, e.getMessage());
				if (retries >= maxRetries) {
					throw new ScrapingException("No se pudo navegar a Consum después de " + maxRetries + " intentos",
							e);
				}
				humanLikeDelay();
			}
	...
	private void acceptCookies(Page page) {
		try {
			...
			String[] cookieSelectors = { "#onetrust-accept-btn-handler", "button[aria-label*='Aceptar']",
	...
	private List<ProductDto> extractProductsFromCurrentPage(Page page) {
		...
		try {
			...
			page.waitForSelector("cmp-widget-product, div.widget-prod",
					new Page.WaitForSelectorOptions().setTimeout(waitForSelectorTimeout));

			// Diferentes selectores posibles para productos
			String[] productSelectors = { "cmp-widget-product", "div.widget-prod" };
	...
	private ProductDto extractProductInfo(Locator productElement) {
		try {
			...
			// Extraer nombre
			try {
				String name = extractText(productElement, "h1.u-title-3", "h1", ".u-title-3");
			...
			// Extraer precio
			try {
				String price = extractText(productElement, ".product-info-price__price", ".price",
						"span.product-info-price__price");
			...
			// Extraer imagen
			try {
				String imageUrl = extractImageUrl(productElement, "img.image-component__image",
			...
			try {
				String productDescription = extractText(productElement, "p.u-size--20");
		...
		for (String selector : selectors) {
			try {
				...
				if (element.isVisible()) {
					return element.textContent().replace("{{ format }}", "").trim();
				}
			} catch (Exception e) {
		...
		for (String selector : selectors) {
			try {
				...
				if (src != null && !src.isEmpty()) {
					return src.startsWith("http") ? src : "https://tienda.consum.es/es" + src;
				}
			} catch (Exception e) {
	...
	private void goToNextPage(Page page) {
		try {
			String[] nextPageSelectors = { ".pagination-next:not(.disabled)", "a.next-page:not(.disabled)" };
